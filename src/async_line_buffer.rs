...
impl AsyncLineBuffer {
    ...
    fn ensure_capacity(&mut self) {
        if self.end >= self.buffer.len() {
            let grow_to = usize::max(self.buffer.len() * 2, self.min_read_size);
    ...
    fn consume_line(&mut self) -> Option<Vec<u8>> {
        if let Some(line_break_pos) = self.line_break_idxs.pop_back() {
            ...
            Some(self.drain_buf_until(diff))
        } else {
            None
        }
    }
...
mod test {
    ...
    fn buffer_handles_single_newline() {
        ...
        async_std::task::block_on(async {
            ...
            assert_eq!("\n".as_bytes(), line.unwrap().text().as_slice());
        });
    }
    ...
    fn buffer_handles_single_byte_str() {
        ...
        async_std::task::block_on(async {
            let line = reader.read_line().await;
            assert_eq!("H".as_bytes(), line.unwrap().text().as_slice());
...
to have had so much blood in him.
        ...
        async_std::task::block_on(async {
            ...
            let line = reader.read_line().await.unwrap();
            assert_eq!(
                "to have had so much blood in him.".as_bytes(),
                line.text().as_slice()
            );
